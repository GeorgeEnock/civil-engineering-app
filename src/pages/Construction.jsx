...
export default function Construction() {
  const section = sidebarNav.find((item) => item.path === '/construction')
  return (
    <div className="space-y-8">
      <section className="rounded-4xl border border-slate-800/70 bg-slate-950/90 p-6 shadow-[0_30px_80px_rgba(0,0,0,0.35)] sm:p-8">
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div>
            <p className="text-sm uppercase tracking-[0.28em] text-slate-500">Construction</p>
            <h1 className="mt-3 text-3xl font-semibold text-white">Construction work packages and delivery</h1>
          ...
          <div className="inline-flex items-center gap-2 rounded-full bg-slate-900/80 px-4 py-2 text-sm text-slate-300">
            <MapPin className="h-4 w-4" /> {section?.items.length} work areas
          </div>
        </div>
